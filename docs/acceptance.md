...
- A content-free production trace on the signed-in iPhone 18 Pro/iOS 27 simulator measured about 6.7 seconds from request arrival to first safe text before the latency change: 1.88 seconds of Workflow queueing and about 4.0 seconds from provider setup through first decoded text. With inline interactive execution and reasoning disabled, the same Nudge flow measured about 2.2 seconds end to end: 0.26 seconds to job start and about 1.15 seconds from provider setup through first text. The reduction is roughly 67% on that observed network, not a guaranteed SLA.
- Live reasoning-off evaluation covered Guided, Practice and Mock interview in text and delegated-voice prompt paths for starting guidance and incorrect retry claims. Responses remained grounded, correctly rejected retry-as-exactly-once, and used zero reasoning tokens. Direct provider timing reached first safe text in 0.76–1.06 seconds across four synthetic streams. API: 104 tests; Swift: 34 tests. No TestFlight upload.

## Native visual system — 14 September 2026

- Unified Home, question preparation/preview, onboarding, restoration, interview actions and Library metadata around semantic native surfaces, monochrome primary actions and shared type, spacing and motion primitives.
