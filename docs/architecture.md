...
Explicit help lives in the workspace menu as Give me a nudge and Show an example. A compact detented sheet opens immediately, shows a native progress indicator while inference runs, then replaces it in place with the one-time result; it never enters the interview document. The server retains the help turn for usage, retry and conservative assistance evidence. Help submissions preserve the unfinished answer and reuse existing revision, recovery and account-scoping contracts.
...
## Teaching modes and bounded voice guidance — 13 September 2026

Learn together, Coach me (default) and Mock interview describe teaching responsibility, independently of the recorded engineering level. A native Practice mode list is available in preparation and the interview menu. Changing it preserves the draft and does not initiate inference. Selection is account/attempt-scoped locally, then committed with the next reply or voice start; active audio must stop before changing modes.

The additive optional `guidanceMode` field appears in generation, interview and voice-start requests, challenge details and interview state. Old clients retain the original style enum; missing mode defaults to Coach me, and omitted fields preserve an already-recorded mode. Generation stores the selected mode. Text jobs capture mode, prompt edition and the bounded historical snapshot with their existing revision/idempotency contract. No D1 migration or new entitlement system.

`interviewer-teaching-v3`, `teaching-v2` and `teaching-v1` separate shared personality from teaching policy. Older explicitly pinned editions remain resolvable. V2 treats an explicit Nudge as grounded, one-time assistance. V3 applies the same delivery model to Show an example, with a slightly larger allowance for one concrete worked slice. Both see the visible question, bounded committed conversation and current saved draft; the compact assistance sheet opens before inference completes and the native document omits the help turn after dismissal while the server retains its assistance record. Voice reasoning uses the same role-based XML context and strict move/text schema as text, with a spoken-delivery instruction; its public API still returns plain `text`. Only visible question requirements enter the new interview model context. The GPT-Live session receives the shared personality, teaching policy and truthful-progress rules. Prompt administration remains deferred; new deployed editions should be added rather than mutating an existing edition.

Voice entry checks server capability and requests microphone permission before changing presentation. Denial leaves the text workspace and draft untouched; later taps recheck permission and offer the system Settings route because iOS cannot repeat its system dialog after denial. The room still requires an explicit Start, so granting permission does not create a paid connection. Routine saves, skips, settings changes and Close use local durable state first and reconcile in the background. Network bookkeeping stays out of normal UI; only semantic conflicts or destructive account transitions require review.
...
### Session-style wording — 13 September 2026

The native selector is now **Session style**: Guided, Practice (default), Mock interview. Display descriptions explain the experience; the persisted `learn_together`, `coach_me`, `mock_interview` identifiers and teaching policies are unchanged. No backend rollout is needed for this copy change. A richer Home is proposed in [Home direction](home-direction.md); that document does not describe shipped layout changes.

### Home practice hub (13 September 2026)

Home prioritizes the existing question, cached practice statistics, one evidence-linked Revisit, and three taxonomy entry points. Searchable topic browsing shares the preparation/preview sheet coordinator. Preparation cannot replace an in-progress interview; topic selections remain per-question. Revisit is derived locally from completed reflection evidence (`needs_practice`, nonempty quote and observation, matching completed source); a source link exposes saved reasoning and dismissal is stored per account/device, keyed to the feedback version. Coverage snapshots use `home-coverage:<account>`; dismissals use `home-revisits:<account>`. They restore before Home publication and clear on account/practice reset. Existing completed-reflection refresh and session deletion invalidate coverage. Home adds no inference calls and no API or database migration.

### Local-first preferences and Home ordering (13 September 2026)

Settings Done persists an account-scoped `PendingSettings` command in `settings-pending:<account>` before dismissal. One background sender coalesces newer edits, leaves failed commands on disk, and retries on bootstrap refresh or explicit Retry. Acknowledgement compares command IDs on the storage actor before clearing; old responses cannot remove newer preferences. Bootstrap/launch overlay pending preferences and reject refreshes started before a local save. Sign-out protects pending preferences with the existing discard flow; practice reset preserves them. This uses the existing PUT contract (last server write wins across devices, no new conflict-resolution contract). Credential/onboarding operations await confirmation, while ordinary Settings does not. Appearance remains device-local AppStorage and changes the currently presented Settings surface immediately.

Home Explore orders topics by fewer completed attempts, then oldest practice, then stable ID. Coverage remains exposure, not mastery. Revisit considers the latest evidence per concept so newer positive evidence retires an older suggestion. This is a conservative UI heuristic, separate from the existing level-specific backend selection rotation (breadth, evidence gap, 14-day revisit, excluding the two most recent topics). Generation retains the bounded eight-attempt narrative snapshot. A comprehensive longitudinal skill model is not implemented.

### General practice personalization and history-v2 (13 September 2026)

Optional `settings.practiceProfile` contains goals, background and preferences, each server-validated to 600 UTF-16 units. Missing fields from old clients preserve the saved profile; explicit empty strings reset it. Native editing uses the existing durable settings outbox. `POST /v1/settings/preview` validates the unsaved profile, authenticates the account, applies a ten-per-day preview budget and existing provider limits, and returns a text sample without saving preferences or creating an attempt. OpenAPI and fixtures describe both additions; no migration.

Personalization is scoped by purpose: generation receives only goals/background, conversation receives all three fields, and reflection receives no profile. Generation/reference payloads also remove raw profile fields so delivery preferences cannot leak through a second path. User context cannot change schemas, grading, visible requirements or teaching responsibility. Text jobs capture context. Voice start accepts an optional current local profile, falling back to saved settings for older clients, and pins it in the existing account-scoped voice job input for both startup and delegated responses. This avoids racing the asynchronous settings outbox. Active audio never reconnects automatically to apply a setting; current spoken requests can override delivery, and the next session captures newer preferences.

History-v2 reads lifetime exposure grouped by level/concept (at most 18 relevant/less-practised rows), eight recent completed/skipped attempts, and up to six latest concept observations from the last 100 completed reflections. Current-question concepts take precedence; observations retain quote/source/date/assistance provenance. Queued context is persisted for retries; cross-request history is intentionally recomputed from bounded D1 queries to avoid stale deletion/reset caches. No additional model summarization pass or inferred biography is introduced.
...
Bootstrap exposes optional `capabilities.developerTools`. The development Worker derives it from an exact account-ID allowlist and hides the owner-only `DELETE /v1/developer/practice` route behind the same check. Reset deletes account-scoped attempts, questions, dependent interview/help/voice data, remaining practice jobs, AI-run history and daily visit claims. It preserves authentication, settings, credentials, devices and usage accounting. Native reset clears matching local practice caches/outboxes before refreshing and preparing a clean daily question. No database migration is required.
