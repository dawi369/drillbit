...
export function interviewSchemaFor(kind: string) {
  return interviewResultSchema.extend({ outcome: ["answer", "continue"].includes(kind) ? z.enum(["follow_up", "wrap_up"]) : z.literal("reply") });
}
export function interviewWrapUp(context: unknown, kind: string) {
  if (kind !== "answer") return null;
  const { interview } = context as { interview: { style: string; turns: {kind: string}[] } };
  const continuation = interview.turns.findLastIndex(t => t.kind === "continue");
  const answers = interview.turns.slice(continuation + 1).filter(t => t.kind === "answer").length;
  const ceiling = continuation >= 0 ? 0 : ({quick:1,standard:2,in_depth:4}[interview.style] ?? 2);
  return answers >= ceiling ? { outcome: "wrap_up" as const, text: "Ready to wrap up? Review this interview, or keep going." } : null;
}
export function normalizeInterviewResult(output: z.infer<typeof interviewResultSchema>) {
  return output.outcome === "wrap_up" ? { ...output, text: "Ready to wrap up? Review this interview, or keep going." } : output;
}
export async function interviewFor(env: Env, account: string, id: string) {
  ...
  const rows = await env.DB.prepare(`SELECT t.*,j.status,j.error FROM interview_turns t JOIN jobs j ON j.id=t.job_id WHERE t.challenge_id=? ORDER BY t.ordinal`).bind(id).all<any>();
  const turns = rows.results.map(t => ({ id: t.id, ordinal: t.ordinal, kind: t.kind, prompt: t.prompt, text: t.text, createdAt: t.created_at, jobId: t.job_id, status: t.status, error: t.error, result: t.result ? JSON.parse(t.result) : null }));
...
export async function requestInterview(env: Env, account: string, id: string, command: string, raw: unknown) {
  ...
  if (!session || session.revision !== input.revision || (input.kind === "answer" && session.answer.trim() !== input.text)) throw new Fault("revision_conflict", 409, "Sync your current answer before sharing.");
  await consumeUsage(env, account, "interview", 50);
  const now = timestamp();
  const payload = JSON.stringify({ settings: await settingsFor(env, account), action: input, turnId: command, context: { question: JSON.parse(challenge.data), interview: context } });
  await env.DB.batch([
