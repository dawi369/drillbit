...
function ChallengeHeaderSkeleton() {
  return (
    <View className="gap-3 pb-3">
      <View className="h-6 w-3/4 rounded-full bg-surface-secondary/70" />
      <View className="gap-2">
        <View className="h-4 w-full rounded-full bg-surface-secondary/60" />
        <View className="h-4 w-5/6 rounded-full bg-surface-secondary/60" />
      </View>
    </View>
  );
}
...
export function AnswerScreen() {
  ...
  const debugMountPayloadRef = useRef({
    params,
    resolvedChallengeId,
  ...
  useEffect(() => {
    const mountPayload = debugMountPayloadRef.current;

    debugLog("answer", "mounted", mountPayload);

    return () => {
      debugLog("answer", "unmounted", {
        resolvedChallengeId: mountPayload.resolvedChallengeId,
        timestamp: Date.now(),
      });
    };
  }, []);
            ...
            <TextField className="flex-1">
              <View className="mb-3 gap-3 px-1">
                <View className="min-w-0 flex-1 gap-0.5">
                  <Label>Answer workspace</Label>
                  <Text
                    className="text-xs font-medium text-muted"
                  >
                    {canInteractWithChallenge
                      ? "Keep the reasoning here. Coach and reveal stay out of the way."
                      : "Open an active challenge to start writing."}
                  </Text>
                </View>
